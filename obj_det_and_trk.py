...
from utils.general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr, set_logging,
...
def draw_boxes(img, bbox, identities=None, names=None):
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for j in range(len(det)):
                    n_id = int(det[j][-1])
                    conf = float(det[j][-2])
                    polygon = pred_poly[j].cpu()
                    #data = [model.names[n_id], (polygon[0], polygon[1]), (polygon[2], polygon[3]), (polygon[4], polygon[5]), (polygon[6], polygon[7])]
                    # x1, y1 = smallest x, smallest y; x2, y2 = biggest x, biggest y
                    x_lst = [polygon[0], polygon[2], polygon[4], polygon[6]]
                    y_lst = [polygon[1], polygon[3], polygon[5], polygon[7]]
                    x_min, x_max = min(x_lst), max(x_lst)
                    y_min, y_max = min(y_lst), max(y_lst)
                    data = [x_min, y_min, x_max, y_max, conf, n_id]
                    datas.append(data)
                ...
                # Use Track Function (undone)
                dets_to_sort = np.empty((0,6))
                for x1,y1,x2,y2,conf,detclass in datas:
